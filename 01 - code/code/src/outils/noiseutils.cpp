...
utils::Image creerPerlin  ( sf::Vector2i    tailleCarte
                          , float           nbrOctave
                          , float           frequence
                          , float           persistence
                          , bool            newSeed )
...
//////////////////////////////////////////
    ...
    //////////////////////////////////////////
    // les déclarations prealables
    module::Perlin                  perlinModule;       ///<  module à charger pour bruit de perlin
    utils::NoiseMap                 mapAltitude;        ///<  la map
    utils::NoiseMapBuilderPlane     heightMapBuilder;   ///<  le constructeur de map
    utils::RendererImage            renderer;           ///<  le renderer de map vers image
    utils::Image                    image;              ///<  l'image dans laquelle on va dessiner la map
//    utils::WriterBMP                writer;             ///<  la classe qui ecrit sur le disque

    //////////////////////////////////////////
    // le perlin
    perlinModule.SetOctaveCount ( nbrOctave );          ///<  nombre d'octaves ( defaut :6 )
    perlinModule.SetFrequency   ( frequence );          ///<  la frequence ( un peu comme la taille )
    perlinModule.SetPersistence ( persistence );        ///<   la persistence gère la rugosité du bruit( mmm un peu comme le constraste)
    if (newSeed){
        srand( time(NULL) );
        perlinModule.SetSeed        ( rand () % 12345 );
    }

    //////////////////////////////////////////
    // creation de la map
    heightMapBuilder.SetSourceModule ( perlinModule );  ///<  on applique le module perlin a notre constructeur de map
    heightMapBuilder.SetDestNoiseMap ( mapAltitude );   ///<  on lui dit dans quel map construir le bruit
    heightMapBuilder.SetDestSize ( tailleCarte.x, tailleCarte.y );            ///<  la taille de la map a construir
    heightMapBuilder.SetBounds (2.0, 6.0, 1.0, 5.0);    ///<  quelle partie du noise utiliser
    heightMapBuilder.Build ();                          ///<  Consrtuire la map telle qu'on l'a definie.

    //////////////////////////////////////////
    // rendu de la map dans une image
    renderer.SetSourceNoiseMap ( mapAltitude );           ///<  on definie la map source du renderer
    renderer.SetDestImage ( image );                      ///<  on definie l'image destination du renderer


//    // modification du gradient
//    renderer.ClearGradient ();                          ///<  on supprime le gradient par defaut (N&B)
//    renderer.AddGradientPoint (-1.0000, utils::Color ( 50,   20,  10, 255 ) );
//    renderer.AddGradientPoint (1.0000, utils::Color  ( 20,  180, 100, 255 ) );

    renderer.Render ();                                 ///<  on rend la map dans l'image
...
sf::Image* creerPerlinImage( sf::Vector2i    tailleCarte
                          , float           nbrOctave
                          , float           frequence
                          , float           persistence
                          , bool            newSeed )
{
    ...
    auto imgNoise = creerPerlin  (  tailleCarte
                          ,            nbrOctave
                          ,            frequence
                          ,            persistence
                          ,            newSeed );
    ...
    std::string     nomFichierImage = "noiseTmp.jpg";

    utils::WriterBMP                writer;             ///<  la classe qui ecrit sur le disque

    //////////////////////////////////////////
    // Ecritutre de l'image sur le disque
    writer.SetSourceImage   ( imgNoise );              ///<  on definie quelle image on veut ecrire
    writer.SetDestFilename  ( nomFichierImage );    ///<  le nom de l'image
    writer.WriteDestFile    ( );                    ///<  on ecrite l'image sur le disque


    sf::Image* image (new sf::Image);
    image->loadFromFile( nomFichierImage );

    remove ( nomFichierImage.c_str() );
...
std::unique_ptr<sf::Texture> creerPerlinTexture( sf::Vector2i    tailleCarte
                          , float           nbrOctave
                          , float           frequence
                          , float           persistence
                          , bool            newSeed )
{
    ...
    utils::Image image = creerPerlin  ( tailleCarte
                          ,             nbrOctave
                          ,             frequence
                          ,             persistence
                          ,             newSeed );
    ...
    //////////////////////////////////////////
    // Ecritutre de l'image sur le disque
    writer.SetSourceImage   ( image );              ///<  on definie quelle image on veut ecrire
    writer.SetDestFilename  ( nomFichierImage );    ///<  le nom de l'image
    writer.WriteDestFile    ( );                    ///<  on ecrite l'image sur le disque
    ...
    if ( ! texture->loadFromFile( nomFichierImage ) )
        std::cout << "RATE !!!\n";
    else std::cout << "reussi\n";
